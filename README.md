...
Streamlitの `更新状態` では、市場別に以下を表示します。
...
- 画面更新: Streamlit左サイドバーの `J-Quants手動更新` / `日本株一括取り込み` / `米国株一括取り込み` / `最新化するまで実行`

Streamlitの `更新状態` では、取り込み率・最新化率、最終成功日時、最終試行日時、対象件数、処理済み件数、スキップ件数、次のoffset、ジョブ履歴を確認できます。

## UI用語ヘルプ

Streamlitのサイドバーに `用語ヘルプ` を用意しています。PER、PBR、ROE、自己資本比率、カタリスト、next_offsetなど、投資やデータ更新で出てくる用語を短い説明と注意点つきで確認できます。主要な入力欄や指標にもヘルプを付けています。
